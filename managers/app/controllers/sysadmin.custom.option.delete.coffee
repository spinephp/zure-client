...
class CustomDeletes extends Spine.Controller
	...
	option: (e)=>
		...
		Custom.one "beforeDestroy", =>
			Custom.url = "woo/"+Custom.url if Custom.url.indexOf("woo/") is -1
			Custom.url += "&token="+ $.fn.cookie('PHPSESSID') unless Custom.url.match /token/
			Custom.url += "&#{field.name}=#{field.value}" for field in key when not Custom.url.match "&#{field.name}="

		Custom.one "ajaxSuccess", (status, xhr) => 
