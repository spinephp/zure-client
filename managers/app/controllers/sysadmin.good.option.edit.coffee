...
class GoodEdits extends Spine.Controller
	...
	uploadGood:(e)->
		e.stopPropagation()
		@uploadFile 'goodimg'+@item.good.classid+"_"+@item.good.id,@filegoodEl[0].files[0],$(@goodimgEl),'images/good/'
	...
	option: (e)->
		...
		for field in key
			...
			if cval isnt ''
				switch field.name[0..1]
					when 'G_'
						item.product[ckey] = cval if cval isnt @item.good[ckey]
					else
		...
		@item.good.scope = 'woo'
		$.ajax
			url: @item.good.url() #"? cmd=Product&token=#{@token}/"+@item.good.id # 提交的页面
			data: param
