...
class GoodclassEdits extends Spine.Controller
	...
	render: ->
		...
		$("body >header h2").text "经营管理->产品类管理->编辑产品类"
	...
	uploadGood:(e)->
		e.stopPropagation()
		@uploadFile 'goodimg'+@item.goodclass.parentid+"_"+@item.goodclass.id,@filegoodEl[0].files[0],$(@goodimgEl),'images/good/'
	...
	waterselClick:(e)->
		e.stopPropagation()
		$(@watermodeEl).css 'display','none'
		$(@watermodeEl).eq($(e.target).val()).css 'display','block'
	...
	option: (e)->
		...
		for field in key
			switch field.name[0..1]
				when 'G_'
					item.good[field.name[2..]] = field.value
				else
					item[field.name] = field.value

		img = $(@goodimgEl).attr 'src'
		name = img.replace 'images/good/',''
		item.good['picture'] = name #if name isnt @item.goodclass.picture

		param = JSON.stringify(item)
		$.ajax
			url: "? cmd=ProductClass&token=#{@token}/"+@item.goodclass.id # 提交的页面
			data: param
